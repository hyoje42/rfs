...
from util import realistic_setting


class ImageNet(Dataset):
    def __init__(self, args, partition='train', pretrain=True, is_sample=False, k=4096,
                 transform=None, is_moco=False, realistic_prob=0.0):
        ...
        self.realistic_prob = realistic_prob
        ...
        if realistic_prob:
            self.imgs, self.labels = realistic_setting(realistic_prob, self.imgs, self.labels)
...
class MetaImageNet(ImageNet):
    ...
    def __init__(self, args, partition='train', train_transform=None, test_transform=None, fix_seed=True, pretrain=False, realistic_prob=0.0):
        super(MetaImageNet, self).__init__(args=args, partition=partition, pretrain=pretrain, realistic_prob=realistic_prob)
    ...
    def __len__(self):
        if self.partition == 'train':
            return len(self.labels) // ((self.n_shots+self.n_queries) * self.n_ways)
        else:
